...
3. **Metadata is editable until "end turn"** - then frozen permanently
4. **One unfrozen item at a time** - human can only work on one item before ending turn
5. **All game history goes into cached context** on every API call
6. **React** - no other framework
7. **User supplies single global API key** - stored in localStorage, used for all AI calls
8. **Support multiple game instances** - user can create/delete/switch between games
...
### v1 (Current Target)
- Single AI player with hard-coded persona
- Human-driven conversation flow (no automatic turns)
- Human clicks "end turn" when satisfied with item
